...
(function (window, undefined) {
    ...
    function _encodeCoords(number) {
        ...
        if (multiple) {//编码倍数位
            second = sample.charAt(multiple);
        }
        abs %= scale; //计算余数
        var result = "";
        if (number < 0) result += "!";
        if (second) result += "$";
        return result + second + sample.charAt(abs);
    };
    //解析数组
    function _decodeArray(code) {
        var secend,
            number,
            array = [];
        for (var i = 0; i < code.length; i++) {
            if (code[i] === '!') {
                ++i;
                if (code[i] === '$') {
                    secend = sample.indexOf(code[++i]);
                    number = sample.indexOf(code[++i]);
                    array.push(-(secend * sample.length + number));
                } else {
                    array.push(-(sample.indexOf(code[i])));
                }
            } else if (code[i] === '$') {
                secend = sample.indexOf(code[++i]);
                number = sample.indexOf(code[++i]);
    ...
    function _decodeCoords(code) {
        var xArray = [],
            ...
            tArry = [],
            tCode;
        var xIndex = code.indexOf("!!");
        var yIndex = code.lastIndexOf("!!");
        xCode = code.substring(0, xIndex);
        yCode = code.substring(xIndex + 2, yIndex);
        tCode = code.substring(yIndex + 2);
        xArray = _decodeArray(xCode);
        yArray = _decodeArray(yCode);
        tArry = _decodeArray(tCode);
        //console.log(xArray, yArray, tArry);
        //for test：重装dragSpotData
        dragSpotData = new Array();
        for (i = 0; i < xArray.length; i++) {
            dragSpotData.push({ 'x': xArray[i], 'y': yArray[i], 'time': tArry[i] });
        }
        console.log(xArray);
    }
    //偏差值检测
    ...
    function _assemblyCoordDate(coords) {
        ...
        for (j = 0; j < coords.length; j++) {
            xArray.push(_encodeCoords(coords[j].x));
            yArray.push(_encodeCoords(coords[j].y));
            tArray.push(_encodeCoords(coords[j].time));
        };
    ...
    function _prepareCanvas() {
        //todo createElement div#vaptchaCanvasDiv dynamicly
    ...
    function _drawTest() {
        ...
        for (var i = 0; i < dragSpotData.length; i++) {
            ...
            context.beginPath();
            if (dragSpotData[i] && i) {
                context.moveTo(dragSpotData[i - 1].x, dragSpotData[i - 1].y);
            } else {
                context.moveTo(dragSpotData[i].x, dragSpotData[i].y);
            }
            context.lineTo(dragSpotData[i].x, dragSpotData[i].y);
            context.closePath();
    ...
    function _addEventHandler() {
        ...
        _eventHandler(document, "mousedown", function (e) {
            // //排除其他鼠标按键
            if (e.which !== 3 && e.button !== 2 && !challenge) return;
            //获取坐标
    ...
    function _generateVaptchaImg(data) {
        ...
        d.appendChild(ValidateImg);
        document.getElementById("VaptchaPoz")[0].appendChild(d);
    }
    //向Vaptcha请求图片
    function _getVaptha(challengeParam, fn) {
        challenge = challengeParam;
        requestAmount += 1;
        getImgCallback = fn
        _getJsonp("./Home/GetImageUrl?callback=IAmAJsonpCallbackName", { "challenge": challengeParam, "siteId": siteId }, "callback", function (data) {
    ...
    vaptcha = {
        ...
        getVaptcha: function (challenge, fn) {
            if (this.challenge && this.challenge.toString().length) {
                ...
                _getVaptha(challenge, fn);
            } else {
                alert("challenge不能为空");
            }
        },
        refreshVaptcha: function (challenge, fn) {
